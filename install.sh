...
function Preference {
	...
	read -p"$GREEN[#NoNemous87#]$WHITE-->" Language
	while [ "$Language" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR GUI-DEFAULT LANGUAGE\n(1)ENGLISH\n(2)ITALIANO\n(3)FRANÇAIS\n(4)BROWSER-LANGUAGE\n\n"
		read -p"$GREEN[#NoNemous87#]$WHITE-->" Language
	...
	read -p"$GREEN[#NoNemous87#]$WHITE-->" Theme
	while [ "$Language" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR GUI-DEFAULT THEME\n(1)LIGHT\n(2)DARK\n(3)HIGH-CONTRAST\n\n"
		read -p"$GREEN[#NoNemous87#]$WHITE-->" Language
...
function Mail_Options {
	...
	read -p"$GREEN[#NoNemous87#]$WHITE-->" Opt
	while [ "$Opt" = "" ];
		do
        printf "${BLUE}\n[?]${WHITE}WOULD YOU LIKE TO ENABLE EMAIL-OPTION(1)YES(2)NO\n\n"
		read -p"$GREEN[#NoNemous87#]$WHITE-->" Opt
	...
	if [ $Opt == 1 ];
		...
		read -p"$GREEN[#NoNemous87#]$WHITE-->" recipient
		while [ "$recipient" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR RECIPIENT EMAIL\n\n"
			read -p"$GREEN[#NoNemous87#]$WHITE-->" recipient
		...
		read -sp"$GREEN[#NoNemous87#]$WHITE-->" password
		while [ "$password" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR EMAIL PASSWORD\n\n"
			read -sp"$GREEN[#NoNemous87#]$WHITE-->" password
		...
		read -p"$GREEN[#NoNemous87#]$WHITE-->" destination
		while [ "$destination" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR DESTINATION EMAIL\n\n"
			read -p"$GREEN[#NoNemous87#]$WHITE-->" destination
		...
		read -p"$GREEN[#NoNemous87#]$WHITE-->" server
		while [ "$server" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR SMTP SERVER EX smtp.test.com\n\n"
			read -p"$GREEN[#NoNemous87#]$WHITE-->" server
		...
		read -p"$GREEN[#NoNemous87#]$WHITE-->" port
		while [ "$port" = "" ];
			do
...
function Options {
	...
	read -sp"$GREEN[#NoNemous87#]$WHITE-->" up_pass
	while [ "$up_pass" = "" ];
		do
        printf "${WHITE}\nINSERT YOUR UPDATE-PASSWORD \n\n"
        read -sp"$GREEN[#NoNemous87#]$WHITE-->" up_pass
	...
	read -p"$GREEN[#NoNemous87#]$WHITE-->" Token
	while [ "$Token" == "" ];
		do
		printf "${WHITE}\n\nWOULD YOU LIKE TO ACCESS YOUR DATABASE ON OTHER DEVICES(ON THE SAME NETWORK)?(1)YES(2)NO\n\n"
		read -p"$GREEN[#NoNemous87#]$WHITE-->" Token
	...
	read -p"$GREEN[#NoNemous87#]$WHITE-->" Access
	while [ "$Access" == "" ];
		do
		printf "${WHITE}\n\nWOULD YOU LIKE TO ADD SOME CREDENTIALS FOR ACCESS THE DATABASE?(1)YES(2)NO\n\n"
		read -p"$GREEN[#NoNemous87#]$WHITE-->" Access
	...
	read -p"$GREEN[#NoNemous87#]$WHITE-->" DateFormat
	while [ "$DateFormat" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR DATE-FORMAT\n(1)EUROPE(DD/MM/YYYY)\n(2)AMERICA'USA'(MM/DD/YYYY)\n(3)ASIA(YYYY/MM/DD)\n\n"
		read -p"$GREEN[#NoNemous87#]$WHITE-->" DateFormat
	...
	cd Configuration
	echo ";THIS FILE HAS BEEN GENERATE BY NoNemous87 INSTALLER">Configuration.ini
...
echo '{
	...
	echo ";THIS FILE HAS BEEN GENERATE BY NoNemous87 INSTALLER">Configuration/Configuration.ini
	...
	echo "password = NoNe87">>Configuration/Configuration.ini
	...
	printf "\n${WHITE}UPDATE-PASSWORD:${GREEN}NoNe87\n"
...
function installer {
	...
	if [ $confvar == 1 ];
		...
		read -p "$GREEN[#NoNemous87#]$WHITE-->" selected
		while [ "$selected" == "" ];
			do
			printf "${BLUE}\n\nWOULD YOU LIKE TO SET(1)MANUAL-INSTALLATION(2)AUTO-INSTALLATION\n\n"
			read -p "$GREEN[#NoNemous87#]$WHITE-->" selected
		...
		printf "${LIGHTGREEN}\n\nTHANK YOU FOR HAVE INSTALLED NoNemous87\n\n"
		exit 0
	fi
