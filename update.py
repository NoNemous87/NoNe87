...
class Downloader:
    ...
    def Update():
        ...
        os.chdir(Path)
        if os.path.exists("NoNemous87_Old"):
            shutil.rmtree("NoNemous87_Old",)
        if os.name == "nt":
            os.system("rename NoNemous87 NoNemous87_Old 2>NUL >NUL")
        else:
            os.system("mv NoNemous87 NoNemous87_Old &>/dev/null")
        os.system("git clone https://github.com/NoNemous87")
        choice = int(input(Font.Color.BLUE + "\n[+]" + Font.Color.WHITE +
                     Language.Translation.Translate_Language(filename, "Update", "Choice", "None")))
        if choice == 1:
            os.remove("NoNemous87_Old")
